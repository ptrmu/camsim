...
int fixed_lag_smoother_example(void) {
  ...
  for(double time = deltaT; time <= 3.0; time += deltaT) {
    ...
    if (time >= 0.50) {
      ...
      Pose2 est = smootherBatch.calculateEstimate<Pose2>(currentKey);
      est.print("Batch Estimate:");
      est = smootherISAM2.calculateEstimate<Pose2>(currentKey);
      est.print("iSAM2 Estimate:");
      cout << endl;
