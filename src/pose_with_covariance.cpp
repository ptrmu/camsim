...
//      dest.setf(std::ios_base::fixed, std::ios_base::floatfield);
  ...
  std::string PoseWithCovariance::to_row_str(gtsam::Vector v)
  {
    std::stringstream ss{};
    NumFmt nf(9, 3);
  ...
  std::string PoseWithCovariance::to_matrix_str(const FixedSizeMatrix &m, bool lower_only)
  {
    std::stringstream ss{};
    NumFmt nf(9, 3);
  ...
  {
    return to_str(pose_).append("\n").append(to_matrix_str(cov_, true));
  }
  ...
  std::string PoseWithCovariance::to_str(const gtsam::Rot3 &rot)
  {
    return to_matrix_str(gtsam::Matrix3{rot.matrix()});
  }
  ...
  {
    return PoseWithCovariance::to_row_str(point);
  }

  std::string PointWithCovariance::to_str(const gtsam::Matrix3 &rot)
  {
    return PoseWithCovariance::to_matrix_str(rot);
  }
  ...
  std::string PointWithCovariance::to_eigenvalues_str(const gtsam::Matrix3 &cov)
  {
    ...
    std::sort(evs.data(), evs.data() + evs.size(), std::greater<>());
    return PoseWithCovariance::to_row_str(evs);
  }

  std::string PointWithCovariance::to_stddev_str(const gtsam::Matrix3 &cov)
  {
    return PoseWithCovariance::to_row_str(cov.diagonal().array().sqrt());
  }
