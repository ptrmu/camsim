...
namespace camsim
{
  ...
  struct MarkerCornersFBoard
  {
    const std::array<gtsam::Point3, 4> marker_corners_f_board_;
  };

  struct MarkerCornersFImage
  {
    const bool visible_;
    const std::array<gtsam::Point2, 4> marker_corners_f_image_;
  };
  ...
  struct CheckerboardsModel
  {
    struct Config
    {
      ...
      Config(bool black_not_white_upper_left, int squares_x, int squares_y, double square_length) :
    ...
    const Config &ch_cfg_;
    const std::vector<BoardModel> boards_;
    const std::vector<gtsam::Point3> junctions_f_board_; // [junction]

    CheckerboardsModel(const Config &ch_cfg_);
  };

  struct CharucoboardsModel : public CheckerboardsModel
  {
    struct Config : public CheckerboardsModel::Config
    {
      const double marker_length_;

      Config(bool black_not_white_upper_left, int squares_x, int squares_y, double square_length,
             double marker_length) :
        CheckerboardsModel::Config{black_not_white_upper_left, squares_x, squares_y, square_length},
        marker_length_{marker_length}
      {}

      Config(const Config &n) :
        CheckerboardsModel::Config{n},
        marker_length_{n.marker_length_}
      {}
    };

    const Config &ar_cfg_;
    const std::vector<MarkerCornersFBoard> corners_f_board; // [marker]

    CharucoboardsModel(const Config &ar_cfg_);
  };

  template<class TBoardsModel>
  struct CalibrationModel : public BaseModel
  {
    const typename TBoardsModel::Config cfg_;
    const TBoardsModel boards_;
    const std::vector<std::vector<std::vector<gtsam::Point2>>> junctions_f_images_; // [camera][board][junction]

    explicit CalibrationModel(const ModelConfig &cfg, const typename TBoardsModel::Config &bd_config);
  };

  struct CheckerboardCalibrationModel : CalibrationModel<CheckerboardsModel>
  {

  };

  struct CharucoboardCalibrationModel : CalibrationModel<CharucoboardsModel>
  {
    const std::vector<std::vector<std::vector<MarkerCornersFImage>>> corners_f_images_; // [camera][board][marker]
